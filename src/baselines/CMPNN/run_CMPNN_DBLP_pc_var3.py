"""
CMPNN link prediction on DBLP paper→conference, variant 3: area connected
to Author rather than to Paper or Conference.
...
def preprocess_dblp_var3(raw_dir, shared_npz, neg_k, seed):
    """Build KG triplets for var3: Area connected to Author."""
    ...
    # var3: area connects to author directly
    valid_author_ids = sorted(author['author_id'].unique())
    ar = author[['author_id', 'label']].rename(columns={'label': 'area_id'}).drop_duplicates()
    ...
    Ar_ids = sorted(ar['area_id'].unique())
    ...
    # var3: author→area (A-R relation)
    for _, row in ar.iterrows():
        a0 = Au_map.get(int(row['author_id']))
        r0 = Ar_map.get(int(row['area_id']))
        if a0 is not None and r0 is not None:
            triplets.append([offA + a0, offR + r0, REL['A-R']])
...
def run_model_DBLP_pc(raw_dir, shared_npz, neg_k, input_dim, hidden_dim,
                      ...
                      checkpoint_path):
    t0 = time.time()
    print('-> Preprocessing DBLP raw data for CMPNN (var3: area↔author) ...')
    triplets, meta, splits = preprocess_dblp_var3(raw_dir, shared_npz, neg_k, base_seed)
...
if __name__ == '__main__':
    ap = argparse.ArgumentParser(description='CMPNN-LP on DBLP paper-conf (var3: area↔author)')
    ...
    ap.add_argument('--save-postfix', default='DBLP_cmpnn_pc_var3')
