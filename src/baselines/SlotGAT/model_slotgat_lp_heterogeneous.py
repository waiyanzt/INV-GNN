"""Multi-node-type SlotGAT encoder for heterogeneous link prediction.

The shared LP protocol supplies a homogeneous DGL graph with globally
...
embedding per node for a task-specific dot-product decoder.
"""
...
class HeterogeneousSlotGATEncoder(nn.Module):
    """Featureless, multi-type SlotGAT encoder for heterogeneous LP."""
    ...
    def __init__(
        ...
        if not torch.equal(unique_types, expected_types):
            raise ValueError(
                "Homogeneous node type IDs must be dense from zero"
            )
