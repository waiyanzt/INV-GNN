# RGCN link prediction on DBLP paper–venue (VAR-2: area connected to Venue)
...
@torch.no_grad()
def eval_ranking(test_pos, test_neg, P, C):
...
def run_one(args, seed):
    ...
    cr = pd.read_csv(os.path.join(args.raw_dir, 'conf_label.txt'),
                     sep=r'\s+|\t', engine='python',
                     names=['conf_id','area_id','conf_name'], header=None)
    cr = cr[cr['conf_id'].isin(Co_map.keys())].copy()
    C2R = (cr['conf_id'].map(Co_map).to_numpy(dtype=np.int64),
           cr['area_id'].map(Ar_map).to_numpy(dtype=np.int64))
    P2C = (m(pc_m,'paper_id',Pa_map),  m(pc_m,'conf_id',Co_map))
    ...
    data = {
        ...
        ('conference','conference-area','area'): C2R,
        ('area','area-conference','conference'): (C2R[1], C2R[0]),
        ('paper','paper-conference','conference'): P2C,
        ('conference','conference-paper','paper'): (P2C[1], P2C[0]),
    }
    ...
    # negatives per paper from split (after optional --neg-per-paper subsampling)
    ...
    for epoch in range(args.epochs):
        enc.train()
        perm = np.random.permutation(len(train_pos))   # shuffle positives only
    ...
    with torch.no_grad():
        ...
        y_pred = (y_prob >= args.th).astype(int)

        # val (for Kendall comparison too)
        vpp = torch.sigmoid((P[S['val_pos'][:,0]] * C[S['val_pos'][:,1]]).sum(-1)).cpu().numpy()
        vpn = torch.sigmoid((P[S['val_neg'][:,0]] * C[S['val_neg'][:,1]]).sum(-1)).cpu().numpy()

    TP = int(((y_pred==1)&(y_true==1)).sum()); FN = int(((y_pred==0)&(y_true==1)).sum())
    TN = int(((y_pred==0)&(y_true==0)).sum()); FP = int(((y_pred==1)&(y_true==0)).sum())

    print("=== RGCN (RelGraphConv, P–C) Var-2: Test ===")
    ...
    print(f"Chosen threshold (from VAL) = {args.th:.3f}")
    ...
    H1,H3,H5,MRR,n = eval_ranking(S['test_pos'], S['test_neg'], P, C)
    n_neg_per = len(S["test_neg"]) // max(1, len(S["test_pos"]))
    print(f"Ranking (1 true + ~{n_neg_per} neg per paper from split) over {n} papers:")
    ...
    return {
        ...
        'F1': f1, 'Accuracy': acc, 'Hits@1': H1, 'Hits@3': H3,
        'Hits@5': H5, 'MRR': MRR
    }
...
def summarize(metrics_list):
    ...
    print("\n===== Summary over seeds =====")
    for k in keys:
        print(f"{k:<8}: {means[k]:.6f} ± {stds[k]:.6f}")

if __name__ == "__main__":
    ap = argparse.ArgumentParser(description="RGCN LP on DBLP var-2 (P–C like MAGNN)")
    ...
    ap.add_argument(
        ...
        help='Max negatives per paper in train/val/test after loading npz. 0 = use full npz lists.',
    )
    ap.add_argument('--th', type=float, default=0.5)

    ap.add_argument('--ckpt', default='checkpoint/rgcn_var2.pt')
    ap.add_argument('--save-postfix', default='DBLP_pv_rgcn_var2')
