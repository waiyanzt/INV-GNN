...
class MAGNN_nc_mb_layer(nn.Module):
    ...
    def forward(self, inputs):
        #print("magnn_nc_mb", inputs[0][0].device)
        # ctr_ntype-specific layers
...
class MAGNN_nc_mb(nn.Module):
    ...
    def forward(self, inputs):
        ...
        for i in range(len(g_list)):
            g_list[i] = g_list[i].to(torch.device(features_list[0].device))
        
        #print(features_list[0].device, "forward", g_list[0].device)
        ...
        for i, fc in enumerate(self.fc_list):
            ...
            #print(f"Processing node type {i}:")
            #print(f"  - Number of nodes for this type: {len(node_indices)}")
            #print(f"  - Shape of features_list[{i}]: {features_list[i].shape}")
            #print(f"  - Shape of transformed_features slice: {transformed_features[node_indices].shape}")
            #print(f"  - Shape of fc(features_list[{i}]): {fc(features_list[i]).shape}")
