...
class HeteroLinearLayer(nn.Module):
    def __init__(self, in_dim_dict, out_dim, bias=True):
        ...
        for ntype, in_dim in in_dim_dict.items():
            self.linears[ntype] = nn.Linear(in_dim, out_dim, bias=bias)
...
class ScorePredictor(nn.Module):
    ...
    def forward(self, edge_subgraph, h_dict):
        with edge_subgraph.local_scope():
            ...
            score = th.sum(edge_subgraph.edges[self.target_etype].data['score'] * self.r, dim=-1)
            return score
...
class LinkPrediction_fullbatch(nn.Module):
    ...
    def forward(self, positive_edges, negative_edges, g, x_dict):
        ...
        pos_score = th.sum(h_u[positive_edges[0]] * h_v[positive_edges[1]] * self.r, dim=-1)
        neg_score = th.sum(h_u[negative_edges[0]] * h_v[negative_edges[1]] * self.r, dim=-1)
        return pos_score, neg_score
