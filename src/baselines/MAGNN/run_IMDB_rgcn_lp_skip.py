...
"""
  python preprocess_IMDB_rgcn_lp_skip.py --task md --variant v1,v3 \\
      --shared-npz ../CMPNN/IMDB_md_shared_splits.npz --neg-k 19

  python run_IMDB_rgcn_lp_skip.py --task md --variants v1,v3 \\
      --seeds 1566911444,20241017,20251017 --save-postfix IMDB_rgcn_lp_skip_md
"""
...
def _parse_variants(s: str, task: str) -> list[str]:
    """Match ``run_IMDB_rgcn_lp.py`` / ``run_IMDB_magnn_lp.parse_variants``."""
...
def base_dir(task: str, variant: str) -> str:
    return f"data/preprocessed/IMDB_rgcn_lp_skip_{task}_{variant}"
...
def main():
    ap = argparse.ArgumentParser(
        description="IMDB RGCN link prediction — skip-node graphs (CMPNN-aligned).",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=(
            "Data must be built first with preprocess_IMDB_rgcn_lp_skip.py "
            "(--shared-npz, --neg-k, --variant live there). "
            "This runner only reads data/preprocessed/IMDB_rgcn_lp_skip_{task}_{variant}/."
        ),
    )
    ...
    ap.add_argument(
        "--variant",
        default=None,
        help="One variant or comma-separated list; split with _parse_variants (do not pass as a single folder name).",
    )
    ap.add_argument("--variants", default="v1,v2,v3,v4", help="Comma-separated variants if --variant omitted.")
    ...
    ap.add_argument("--ckpt", default="checkpoint/imdb_rgcn_lp_skip.pt")
    ap.add_argument("--seeds", default="1566911444,20241017,20251017")
    ap.add_argument("--save-postfix", default="IMDB_rgcn_lp_skip")
    ...
    if args.compare_only:
        ...
        print(f"\n########## Kendall τ | IMDB LP skip task={task} (pairwise) ##########")
    ...
    for v in variants:
        print(f"\n########## IMDB RGCN LP skip {task} {v} ##########", flush=True)
        runs = []
        for sd in seeds:
    ...
    print(f"\nIMDB RGCN LP skip summary | task={task} | mean ± std over seeds")
