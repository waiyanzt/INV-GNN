...
Canonical semantic graph saved for ALL variants:
  movie <-> director
  movie <-> actor
  movie <-> link

Skip-node meaning:
- v2: movie-director and movie-actor may be recovered through link
- v3: movie-actor recovered through link
- v4: movie-director recovered through link

If the recovered canonical semantic edges are identical across v1-v4,
RGCN sees the same graph and should produce the same results under deterministic training.

to run the preprocessing:
python preprocess_IMDB_rgcn_skip.py --variant v1
python preprocess_IMDB_rgcn_skip.py --variant v2
python preprocess_IMDB_rgcn_skip.py --variant v3
python preprocess_IMDB_rgcn_skip.py --variant v4
...
def preprocess_variant(variant):
    ...
    raw = build_raw_edges_for_variant(movies, director_to_idx, actor_to_idx, link_to_idx, info, variant)
    semantic = collapse_to_canonical_semantic_edges(raw, variant)
    ...
    np.save(os.path.join(out_dir, "type_mask.npy"), info["type_mask"])
    ...
    print(f"[{variant}] semantic edge counts:")
    for k in ["movie_to_director", "movie_to_actor", "movie_to_link"]:
        print(f"  {k}: {len(semantic[k])}")
...
if __name__ == "__main__":
    ...
    set_seed(SEED)
    preprocess_variant(args.variant)