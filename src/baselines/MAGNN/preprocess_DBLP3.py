# DBLP paper–venue LP preprocessing (Var3: area-author)
...
OUT = 'data/preprocessed/DBLP_lp_pc_var3_ct_from_pt/'
RAW = 'data/raw/DBLP/'
...
for p,c in pc[['paper_id','conf_id']].itertuples(index=False):
    if p in Pa_map and c in Co_map:
        ...
        adjM[ci,pi] = 1
# A–R / 0-4
for a,r in author[['author_id','label']].itertuples(index=False):
    if a in Au_map and r in Ar_map:
        ai,ri = 0+Au_map[a], A+P+T+C+Ar_map[r]
        adjM[ai,ri] = 1
        adjM[ri,ai] = 1
