...
from utils.data import load_DBLP_lp_pc_var3_data
...
etypes_lists = [
    [[1,0], [2,3], [1,6,7,0], [4,5]],          # paper-centric
...
def run_model_DBLP_pv_var3(feats_type, hidden_dim, num_heads, attn_vec_dim, rnn_type,
    ...
    print("→ Loading preprocessed DBLP paper–venue data (var3: area↔author) ...")
    t0 = time.time()
    (adjlists, edge_metapath_indices_list, _, type_mask,
     pos_splits, neg_splits, num_paper, num_conf) = load_DBLP_lp_pc_var3_data(expected_metapaths)
    ...
    # Metric collectors (across repeats)
    auc_list, ap_list = [], []
    ...
    for rep in range(repeat):
        ...
        print('Link Prediction Test (var3)')
        print(f'AUC = {auc:.6f}')
        print(f'AP  = {ap:.6f}')
    ...
    # Return aggregated stats for the seed run
    return {
...
if __name__ == '__main__':
    ap = argparse.ArgumentParser(description='MAGNN-LP on DBLP paper–venue (var3: area↔author)')
    ...
    ap.add_argument('--save-postfix', default='DBLP_pv_var3')
    ...
    ap.add_argument('--seeds', default='1566911444,20241017,20251017',
                    help='Comma-separated list of seeds for multiple runs')
    ap.add_argument('--neg-mult', type=int, default=3,
                    help='Number of negative samples per positive (training/validation).')
    ap.add_argument(
        '--test-neg-per-paper',
        type=int,
        default=3,
    ...
    for seed in seeds:
        ...
        stats = run_model_DBLP_pv_var3(args.feats_type, args.hidden_dim, args.num_heads, args.attn_vec_dim,
                                       args.rnn_type, args.epoch, args.patience, args.batch_size,
                                       args.samples, 1,  # repeat=1 per seed
