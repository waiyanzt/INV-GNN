...
def get_metapath_neighbor_pairs(M, type_mask, expected_metapaths):
    ...
    for metapath in expected_metapaths:
        ...
        metapath_to_target = {}
        source_nodes = (type_mask == metapath[0]).nonzero()[0]
        target_nodes = (type_mask == metapath[(len(metapath) - 1) // 2]).nonzero()[0]
        
        # Add tqdm progress bar here
        for source in tqdm(source_nodes, desc=f"Processing sources for {metapath}"):
...
save_prefix = 'data/preprocessed/DBLP_preprocessed_area/'
num_ntypes = 4
...
#paper_area = pd.read_csv('data/raw/DBLP/paper.txt', sep='\t', header=None, names=['paper_id', 'paper_title'], keep_default_na=False, encoding='cp1252')
...
# Keep only conf_id 36 and 2180 (AAAI and IJCAI)
keep_confs = [36, 2180]
print(f"Keeping {len(keep_confs)} conferences (ID and Names):", [(cid, confs[confs['conf_id'] == cid]['conf'].values[0]) for cid in keep_confs])
...
print(f"Original author count: {len(labeled_authors)}")
print(f"Filtered author count: {len(author_label)}")

aunique = {}
pa = {'paper_id': [], 'area_id': []}
for _, row in author_label.iterrows():
    aunique[row['author_id']] = 0
for author_id in aunique: 
    paper_id = paper_author[paper_author['author_id'] == author_id].iloc[0]['paper_id']
    pa['paper_id'].append(paper_id)
    area_id = author_label[author_label['author_id'] == author_id].iloc[0]['label']
    pa['area_id'].append(area_id)

'''pa = {'paper_id': [], 'area_id': []}
for _, row in papers.iterrows():
    pa['paper_id'].append(row['paper_id'])
for p in pa['paper_id']:
    author_id = paper_author[paper_author['paper_id'] == p].iloc[0]['author_id']
    area_id = author_label[author_label['author_id'] == author_id].iloc[0]['label']
    pa['area_id'].append(area_id)'''

paper_area = pd.DataFrame.from_dict(pa)
...
print(f"{'Label':<10}{'Conference ID':<15}{'Conference Name':<25}{'Count':<10}")
print("-" * 65)

label_counts = paper_conf['conf_id'].value_counts().sort_index()
confs_sorted = confs.sort_values(by='conf_id')  # Ensure it's aligned by label
conf_ids = confs_sorted['conf_id'].values
conf_names = confs_sorted['conf'].values

for i, count in enumerate(label_counts):
    print(f"{i:<10}{conf_ids[i]:<15}{conf_names[i]:<25}{count:<10}")
...
# 0 for authors, 1 for papers, 2 for terms, 3 for areas
...
for metapath in [(0, 1, 0), (0, 2, 0), (0, 3, 0)]:
    ...
    for target_idx in target_idx_list:
        ...
        #print(target_metapaths_mapping[target_idx])
