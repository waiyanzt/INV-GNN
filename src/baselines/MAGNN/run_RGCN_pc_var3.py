# RGCN link prediction on DBLP paper–venue (VAR-3: area connected to Author)
...
def run_one(args, seed):
    ...
    # VAR-3: author <-> area
    ar = al[['author_id','label']].drop_duplicates()
    tmp = ar.assign(a=ar['author_id'].map(Au_map), r=ar['label'].map(Ar_map)).dropna()
    A2R = (tmp['a'].astype(np.int64).to_numpy(), tmp['r'].astype(np.int64).to_numpy())

    # use all observed P–C (across splits) as graph supervision
    P2C_all = (S['all_pos'][:,0], S['all_pos'][:,1])
    ...
    data = {
        ...
        ('author','author-area','area'): A2R,
        ('area','area-author','author'): (A2R[1], A2R[0]),
    ...
    with torch.no_grad():
        ...
        # Also compute validation scores (for Kendall csv parity)
    ...
    print("=== RGCN (RelGraphConv, P–C) Var-3: Test ===")
...
if __name__ == "__main__":
    ap = argparse.ArgumentParser(description="RGCN LP on DBLP var-3 (P–C like MAGNN)")
    ...
    ap.add_argument('--ckpt', default='checkpoint/rgcn_var3.pt')
    ap.add_argument('--save-postfix', default='DBLP_pv_rgcn_var3')
