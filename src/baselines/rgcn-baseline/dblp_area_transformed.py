...
class DBLP_areaTransformedDataset(DGLDataset):
    def __init__(self):
        super().__init__(name='DBLP_area_transformed')

    def process(self):
        path = '../MAGNN/data/csv/DBLP_area_transformed/'
        ...
        author_area = pd.read_csv(os.path.join(path, 'author_area.csv'))
        ...
        for _, row in paper_term.iterrows():
            src.append(row['src_id'])
            dst.append(row['dst_id'])
        for _, row in author_area.iterrows():
            src.append(row['src_id'])
            dst.append(row['dst_id'])

        g = dgl.graph((src, dst), num_nodes=num_nodes)

        # Dynamically detect feature size
        paper_feat_dim = len(paper['feat'].iloc[0].split(','))
        features = torch.zeros((num_nodes, paper_feat_dim))
